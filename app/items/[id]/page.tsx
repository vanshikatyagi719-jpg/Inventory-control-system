    ...
    >
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '13px', fontWeight: 700, fontFamily: 'monospace', backgroundColor: '#f1f5f9', padding: '3px 8px', borderRadius: '4px' }}>
                  {item.sku}
                </span>
                <span style={{ backgroundColor: '#e0e7ff', color: '#3730a3', fontSize: '12px', fontWeight: 600, padding: '3px 8px', borderRadius: '4px' }}>
                  {item.category_name}
                </span>
                {item.is_archived ? (
                  <span style={{ backgroundColor: '#f1f5f9', color: '#64748b', fontSize: '12px', fontWeight: 600, padding: '3px 8px', borderRadius: '4px' }}>
                    Archived
                  </span>
                ) : (
                  <span style={{ backgroundColor: '#ecfdf5', color: '#047857', fontSize: '12px', fontWeight: 600, padding: '3px 8px', borderRadius: '4px' }}>
                    Active
                  </span>
                )}
              </div>

              <h1 style={{ margin: '10px 0 6px', fontSize: '24px', fontWeight: 800, color: '#0f172a' }}>{item.name}</h1>
              {item.description && (
                <p style={{ margin: 0, color: '#64748b', fontSize: '14px', maxWidth: '650px' }}>
                  {item.description}
                </p>
              )}
            </div>
          ...
          >
            <div>
              <span style={{ fontSize: '11px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Total On-Hand</span>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginTop: '4px' }}>
                <span style={{ fontSize: '26px', fontWeight: 800, color: item.is_low_stock ? '#dc2626' : '#0f172a' }}>
                  {item.total_on_hand}
                </span>
                <span style={{ fontSize: '13px', color: '#64748b' }}>{item.unit_of_measure}</span>
              </div>
            </div>

            <div>
              <span style={{ fontSize: '11px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reorder Level</span>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginTop: '4px' }}>
            ...
            <div>
              <span style={{ fontSize: '11px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Alert Status</span>
              <div style={{ marginTop: '8px' }}>
                {item.is_low_stock ? (
                  <span style={{ backgroundColor: '#fee2e2', color: '#b91c1c', padding: '4px 10px', borderRadius: '4px', fontSize: '12px', fontWeight: 700 }}>
                    AT / BELOW REORDER LEVEL
                  </span>
                ) : (
                  <span style={{ backgroundColor: '#ecfdf5', color: '#047857', padding: '4px 10px', borderRadius: '4px', fontSize: '12px', fontWeight: 700 }}>
                    Healthy Stock
                  </span>
                )}
              </div>
        ...
        >
          <h2 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Stock On-Hand by Location</h2>
            ...
            {locationStock.map((loc) => (
              ...
              >
                ...
                <div style={{ fontSize: '20px', fontWeight: 800, color: '#0f172a', marginTop: '4px' }}>
                  {loc.onHand} <span style={{ fontSize: '12px', fontWeight: 400, color: '#64748b' }}>{item.unit_of_measure}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '24px' }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Stock Movement Ledger (Append-Only)</h2>
                <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                  Permanent record of receipts, issues, transfers, and adjustments.
                </p>
              </div>
              <Link
            ...
            {movements.length === 0 ? (
              <p style={{ padding: '24px', textAlign: 'center', color: '#94a3b8', margin: 0 }}>
                      ...
                      return (
                        <tr key={mov.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          ...
                          <td style={{ padding: '10px 12px' }}>
                            <span style={{ backgroundColor: typeBg, color: typeColor, padding: '2px 8px', borderRadius: '4px', fontWeight: 700, fontSize: '11px', textTransform: 'uppercase' }}>
                              {mov.movement_type}
                            </span>
                          </td>
          ...
          >
            <h2 style={{ margin: '0 0 6px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>
              Item Audit Timeline & Staff Notes
            </h2>
            <p style={{ margin: '0 0 20px', fontSize: '12px', color: '#64748b' }}>
              Every field change, creation event, and staff note is permanently audited.
            </p>
            ...
            <form action={handleAddNote} style={{ display: 'flex', gap: '10px', marginBottom: '24px' }}>
              <input
                ...
                name="note"
                required
                placeholder="Leave an immutable note about this item..."
                style={{
                  flex: 1,
                  padding: '9px 14px',
                  borderRadius: '6px',
              ...
              {timelineEvents.map((evt) => (
                <div
                  ...
                  style={{
                    ...
                    gap: '14px',
                    padding: '14px',
                    backgroundColor: evt.type === 'note' ? '#f0fdf4' : '#f8fafc',
                    borderRadius: '6px',
